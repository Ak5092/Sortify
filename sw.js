/* ═══════════════════════════════════════════════
   Sortify Service Worker — v3 PRODUCTION
...
async function generateIcon(size) {
  try {
    ...
    bg.addColorStop(0, '#080808'); bg.addColorStop(1, '#0e0e0e');
    ctx.fillStyle = bg;
    const r = size * 0.22;
    ctx.beginPath();
    ...
    const glow = ctx.createRadialGradient(cx, cy*.8, 0, cx, cy*.8, size*.55);
    glow.addColorStop(0, 'rgba(121,227,255,.25)');
    glow.addColorStop(.5, 'rgba(255,108,144,.1)');
    glow.addColorStop(1, 'transparent');
    ctx.fillStyle = glow; ctx.fillRect(0, 0, size, size);
    ctx.save();
    const g = ctx.createLinearGradient(0, 0, size, size);
    g.addColorStop(0, '#79e3ff'); g.addColorStop(1, '#ff6c90');
    ctx.strokeStyle = g; ctx.lineWidth = size * .028; ctx.lineCap = 'round';
    const rr = size * .32, circ = 2 * Math.PI * rr;
